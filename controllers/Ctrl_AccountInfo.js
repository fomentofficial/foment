      ...
      connection.query(getUserIDQuery, [templateURL], (error, results) => {
        ...
        for (const group of dataArray) {
          ...
          for (const inner of innerData) {
            ...
            // 데이터를 사용하여 원하는 작업 수행
            console.log(`계좌그룹: ${accordionTitle}`);
            console.log(`은행명: ${bankNameTerms}`);
            console.log(`예금주: ${holderInfo}`);
            console.log(`은행명: ${bankBankName}`);
            console.log(`계좌번호: ${bankAccountInfo}`);
            console.log('---');
