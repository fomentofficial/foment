...
let CreateTemplate = {
  ...
  postInfo: (req, res) => {
    ...
    // account 생성 쿼리
    const insertAccount = 'INSERT INTO account (user_ID, template_ID, accordionTitle, bankNameTerms, holderInfo, bankBankName, bankAccountInfo) VALUES (?, ?, ?, ?, ?, ?, ?)';
              ...
              if (err) {
                ...
                Templatedata.Templatedata.data.accountData.forEach((account) => {
                  ...
                  innerData.forEach((innerItem) => {
                    ...
                    connection.query(insertAccount, [user_ID, template_ID, accordionTitle, bankNameTerms, holderInfo, bankBankName, bankAccountInfo], (error, results, fields) => {
                      if (error) {
                        console.error(error);
                        res.status(500).send('서버 에러');
                      } else {
                        console.log('AccountData 전송 성공');
                      }
                    });
                  });
                });
